...
class StringFieldTest {
	...
	void serializeDeserialize() {
		...
		Record rec1 = new Record(Arrays.asList(aval, bval), schema);

		IntField cval = new IntField(2);
		StringField dval = new StringField("bar", st);
		Record rec2 = new Record(Arrays.asList(cval, dval), schema);
		...
		rec1 = new Record(Arrays.asList(aval, new StringField("", st)), schema);
		rec1.serialize(buf, 0);
		rec2.deserialize(buf,  0);
		assertTrue(rec1.equals(rec2));
	}
